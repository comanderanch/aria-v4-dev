#!/usr/bin/env python3
"""
ARIA Round 18 — Continue from Transplant State
===============================================
NOT standard. NOT scared.

Starts from round17_pass3_best.pt at 2.731170.
No shock phase. No transplant rebuild.
The model has adapted. Now we descend.

Farmer logic — three passes, two rewinds:
  --epochs-pass1    epochs for Pass 1
  --epochs-rewind1  epochs for Rewind 1
  --epochs-pass2    epochs for Pass 2
  --epochs-rewind2  epochs for Rewind 2
  --epochs-pass3    epochs for Pass 3

Every pass saved unconditionally.
Floor gate removed — progress never lost.
...
parser.add_argument("--start",          type=str,   default="round17_pass3_best.pt")
...
total_epochs = (args.epochs_pass1 + args.epochs_rewind1 +
                ...
                args.epochs_pass3)
est_minutes  = total_epochs * 0.006  # ~0.006 min/epoch measured

print()
print("╔══════════════════════════════════════════════╗")
print("║    ARIA — ROUND 18 — CONTINUE DESCENT       ║")
print("║  Starts from transplant-adapted state.      ║")
print("║  No shock. Every pass saved.                ║")
...
pass1_path  = ckpt_dir / "round18_pass1_best.pt"
pass2_path  = ckpt_dir / "round18_pass2_best.pt"
pass3_path  = ckpt_dir / "round18_pass3_best.pt"
rw1_path    = ckpt_dir / "round18_rewind1_best.pt"
rw2_path    = ckpt_dir / "round18_rewind2_best.pt"
...
if not start_path.exists():
    print(f"ERROR: start checkpoint not found — {start_path}")
    sys.exit(1)
...
# Saves unconditionally — no floor gate
# ═══════════════════════════════════════════════
...
print(f"Starting Round 18 — no shock — descent from {prev_loss:.6f}")
...
loss1, state1 = run_pass(model, loader, ep1,
                          f"PASS 1 — {ep1} epochs",
...
if loss1 < floor:
    ...
    torch.save({"model_state": state1, "best_loss": floor,
                "note": "Round 18 Pass 1 — FLOOR BROKEN"}, word_path)
...
_, rewind_state1 = run_pass(model_rw1, loader, erw1,
                             f"REWIND 1 — {erw1} epochs",
...
loss2, state2 = run_pass(model2, loader, ep2,
                          f"PASS 2 — {ep2} epochs",
...
if loss2 < floor:
    ...
    torch.save({"model_state": state2, "best_loss": floor,
                "note": "Round 18 Pass 2 — FLOOR BROKEN"}, word_path)
    print(f"  FLOOR BROKEN — word checkpoint updated: {floor:.6f}")
...
_, rewind_state2 = run_pass(model_rw2, loader, erw2,
                             f"REWIND 2 — {erw2} epochs",
...
loss3, state3 = run_pass(model3, loader, ep3,
                          f"PASS 3 — {ep3} epochs",
...
if loss3 < floor:
    ...
    torch.save({"model_state": state3, "best_loss": floor,
                "note": "Round 18 Pass 3 — FLOOR BROKEN"}, word_path)
    print(f"  FLOOR BROKEN — word checkpoint updated: {floor:.6f}")
...
if broke_floor:
    ...
    print(f"Round 19 loads: {pass3_path.name}  ({loss3:.6f})")
print()
print("No shock. Adapted weights. Farmer logic.")
