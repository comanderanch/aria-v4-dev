#!/usr/bin/env python3
"""
ARIA Round 22 — EXTENDED DESCENT
==================================
Starts from round21_pass3_best.pt at 3.980421.
578 new embeddings initialized. Shock over.
Descent accelerates now.

Pass 1: 50 | Rewind: 20 | Pass 2: 75 | Rewind: 25 | Pass 3: 100
Target: break 3.0 — push toward 2.671585 and below.
...
parser.add_argument("--start",          type=str,   default="round21_pass3_best.pt")
...
total_epochs = (args.epochs_pass1 + args.epochs_rewind1 +
                ...
                args.epochs_pass3)
est_minutes  = total_epochs * 2.0   # ~2 min/epoch at 96k sequences

print()
print("╔══════════════════════════════════════════════╗")
print("║   ARIA — ROUND 22 — EXTENDED DESCENT        ║")
print("║   Shock over. 578 embeddings initialized.   ║")
print("║   Target: break 3.0 — push to 2.671.       ║")
...
pass1_path = ckpt_dir / "round22_pass1_best.pt"
pass2_path = ckpt_dir / "round22_pass2_best.pt"
pass3_path = ckpt_dir / "round22_pass3_best.pt"
rw1_path   = ckpt_dir / "round22_rewind1_best.pt"
rw2_path   = ckpt_dir / "round22_rewind2_best.pt"
...
def run_pass(model, loader, epochs, pass_name, optimizer, scheduler,
    ...
    torch.save({"model_state": best_state, "best_loss": best_loss,
                "pass": pass_name, "note": f"Round 22 — {pass_name}"}, save_path)
...
print(f"Starting Round 22 — descent from {prev_loss:.6f}")
print("Farmer logic. Three passes. Two rewinds.")
print("Shock over. Embeddings initialized. Descent accelerates.")
print()
...
if loss1 < floor:
    ...
    torch.save({"model_state": state1, "best_loss": floor,
                "note": "Round 22 Pass 1 — FLOOR BROKEN"}, word_path)
...
if loss2 < floor:
    ...
    torch.save({"model_state": state2, "best_loss": floor,
                "note": "Round 22 Pass 2 — FLOOR BROKEN"}, word_path)
    print(f"  FLOOR BROKEN: {floor:.6f}")
...
if loss3 < floor:
    ...
    torch.save({"model_state": state3, "best_loss": floor,
                "note": "Round 22 Pass 3 — FLOOR BROKEN"}, word_path)
    print(f"  FLOOR BROKEN: {floor:.6f}")
...
if broke_floor:
    ...
    print(f"Round 23 loads: {pass3_path.name}  ({loss3:.6f})")
print()
