#!/usr/bin/env python3
"""
ARIA Round 27 — LANGUAGE FIX PASS
====================================
...
print()
print("╔══════════════════════════════════════════════╗")
print("║   ARIA — ROUND 27 — LANGUAGE FIX PASS       ║")
...
save_path = ckpt_dir / "round27_best.pt"

candidates = [
    ckpt_dir / "round26_best.pt",
...
# ═══════════════════════════════════════════════
EPOCHS = 3
opt    = optim.SGD(model.parameters(), lr=0.0001, momentum=0.9, weight_decay=1e-4)
sch    = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=EPOCHS, eta_min=1e-7)

print("=" * 60)
print(f"ROUND 27 — {EPOCHS} epochs — Consolidation Pass — lower LR — fewer UNK")
...
torch.save({
    ...
    "best_loss":   best_loss,
    "pass":        "Round 27 — Language Fix — Vocabulary 1632",
...
print()
print("=" * 60)
print(f"Round 27 complete — {elapsed/60:.1f} min")
print(f"Start loss:        {prev_loss:.6f}")
