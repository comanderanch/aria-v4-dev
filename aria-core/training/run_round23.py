#!/usr/bin/env python3
"""
ARIA Round 23 — TOKEN TRAIL ACTIVE
=====================================
Starts from round22_pass3_best.pt.
Token Trail Diagnostic System wired in.
Every activation logged. Every breakthrough sealed.
...
parser.add_argument("--start",          type=str,   default="round22_pass3_best.pt")
...
print()
print("╔══════════════════════════════════════════════╗")
print("║   ARIA — ROUND 23 — TOKEN TRAIL ACTIVE      ║")
print("║   Every activation logged.                  ║")
print("║   Every breakthrough sealed.                ║")
print("║   The plow shows its work.                  ║")
...
pass1_path = ckpt_dir / "round23_pass1_best.pt"
pass2_path = ckpt_dir / "round23_pass2_best.pt"
pass3_path = ckpt_dir / "round23_pass3_best.pt"
rw1_path   = ckpt_dir / "round23_rewind1_best.pt"
rw2_path   = ckpt_dir / "round23_rewind2_best.pt"
...
print(f"Starting Round 23 — descent from {prev_loss:.6f}")
print("Farmer logic. Three passes. Two rewinds.")
print("Token trail active. Every row visible.")
print()
...
if loss1 < floor:
    ...
    torch.save({"model_state": state1, "best_loss": floor,
                "note": "Round 23 Pass 1 — FLOOR BROKEN"}, word_path)
...
if loss2 < floor:
    ...
    torch.save({"model_state": state2, "best_loss": floor,
                "note": "Round 23 Pass 2 — FLOOR BROKEN"}, word_path)
    print(f"  FLOOR BROKEN: {floor:.6f}")
...
if loss3 < floor:
    ...
    torch.save({"model_state": state3, "best_loss": floor,
                "note": "Round 23 Pass 3 — FLOOR BROKEN"}, word_path)
    print(f"  FLOOR BROKEN: {floor:.6f}")
...
if broke_floor:
    ...
    print(f"Round 24 loads: {pass3_path.name}  ({loss3:.6f})")
print()
print("Token trail sealed: /tmp/aria-token-trail.jsonl")
print("  python3 aria-core/diagnostics/token_trail.py --round 23 --all")
print()
