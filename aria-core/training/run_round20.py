#!/usr/bin/env python3
"""
ARIA Round 20 — CALIBRE CORPUS ASSAULT
========================================
Starts from round19_pass3_best.pt at 2.671585.
5 million words. 1566 books. The wall-breaker.
Previous floor: 2.465939.
...
parser.add_argument("--epochs-pass1",   type=int,   default=5)
parser.add_argument("--epochs-rewind1", type=int,   default=2)
parser.add_argument("--epochs-pass2",   type=int,   default=8)
parser.add_argument("--epochs-rewind2", type=int,   default=3)
parser.add_argument("--epochs-pass3",   type=int,   default=15)
...
total_epochs = (args.epochs_pass1 + args.epochs_rewind1 +
                ...
                args.epochs_pass3)
est_minutes  = total_epochs * 4.0   # ~4 min/epoch with 157k sequences

print()
print("╔══════════════════════════════════════════════╗")
print("║   ARIA — ROUND 20 — CALIBRE ASSAULT         ║")
print("║   5 million words. 1566 books.              ║")
print("║   The wall breaks here.                     ║")
...
pass1_path = ckpt_dir / "round20_pass1_best.pt"
pass2_path = ckpt_dir / "round20_pass2_best.pt"
pass3_path = ckpt_dir / "round20_pass3_best.pt"
rw1_path   = ckpt_dir / "round20_rewind1_best.pt"
rw2_path   = ckpt_dir / "round20_rewind2_best.pt"
...
# ── CALIBRE CORPUS (primary) ────────────────────
calibre_corpus = Path(__file__).parent / "calibre_corpus.txt"
training_dir   = Path(__file__).parent
aria_dir       = training_dir.parent

if calibre_corpus.exists():
    size_mb = calibre_corpus.stat().st_size / (1024 * 1024)
    print(f"  Calibre corpus: {calibre_corpus.name} ({size_mb:.1f} MB)")
    corpus_text = calibre_corpus.read_text(encoding='utf-8', errors='replace')
else:
    # Fallback: existing 4-file corpus
    print("  WARNING: calibre_corpus.txt not found — using 4-file fallback")
    paths = [
...
loader  = torch.utils.data.DataLoader(
    dataset, batch_size=BATCH_SIZE_TRAINING, shuffle=True, num_workers=4,
    pin_memory=True
)
...
def run_pass(model, loader, epochs, pass_name, optimizer, scheduler,
    ...
    torch.save({"model_state": best_state, "best_loss": best_loss,
                "pass": pass_name, "note": f"Round 20 — {pass_name}"}, save_path)
...
print(f"Starting Round 20 — Calibre assault from {prev_loss:.6f}")
print("Farmer logic. Three passes. Two rewinds.")
print(f"5 million words feeding the field.")
print()
...
if loss1 < floor:
    ...
    torch.save({"model_state": state1, "best_loss": floor,
                "note": "Round 20 Pass 1 — FLOOR BROKEN"}, word_path)
...
if loss2 < floor:
    ...
    torch.save({"model_state": state2, "best_loss": floor,
                "note": "Round 20 Pass 2 — FLOOR BROKEN"}, word_path)
    print(f"  FLOOR BROKEN: {floor:.6f}")
...
if loss3 < floor:
    ...
    torch.save({"model_state": state3, "best_loss": floor,
                "note": "Round 20 Pass 3 — FLOOR BROKEN"}, word_path)
    print(f"  FLOOR BROKEN: {floor:.6f}")
...
if broke_floor:
    ...
    print(f"Round 21 loads: {pass3_path.name}  ({loss3:.6f})")
print()
