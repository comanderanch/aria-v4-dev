#!/usr/bin/env python3
"""
ARIA Round 24 — VOCABULARY EXPANSION ROUND
============================================
Starts from round23_pass3_best.pt (loss 3.935445).
Token Trail active with training_top5_planes.
Lord log drift tracking live from epoch 1.
Gap 2.33 target: UNK(7.62) - pos2(5.29) — watch the slope.
...
parser.add_argument("--start",          type=str,   default="round23_pass3_best.pt")
parser.add_argument("--epochs-pass1",   type=int,   default=50)
parser.add_argument("--epochs-rewind1", type=int,   default=20)
parser.add_argument("--epochs-pass2",   type=int,   default=75)
parser.add_argument("--epochs-rewind2", type=int,   default=25)
parser.add_argument("--epochs-pass3",   type=int,   default=100)
...
total_epochs = (args.epochs_pass1 + args.epochs_rewind1 +
                ...
                args.epochs_pass3)
est_minutes  = total_epochs * 2.0

print()
print("╔══════════════════════════════════════════════╗")
print("║   ARIA — ROUND 24 — VOCABULARY EXPANSION    ║")
print("║   Token Trail + training_top5_planes live.  ║")
print("║   Lord log drift from epoch 1.              ║")
print("║   Gap 2.33 — watch the slope close.         ║")
...
pass1_path = ckpt_dir / "round24_pass1_best.pt"
pass2_path = ckpt_dir / "round24_pass2_best.pt"
pass3_path = ckpt_dir / "round24_pass3_best.pt"
rw1_path   = ckpt_dir / "round24_rewind1_best.pt"
rw2_path   = ckpt_dir / "round24_rewind2_best.pt"
...
print("  training_top5_planes live from epoch 1 (GPT denominator fix)")
...
def run_pass(model, loader, epochs, pass_name, optimizer, scheduler,
    ...
    for epoch in range(1, epochs + 1):
        ...
        for inputs, targets in loader:
            ...
            # Keep last batch for trail logging
        ...
        # Token trail — log activation pattern
    ...
    torch.save({"model_state": best_state, "best_loss": best_loss,
                "pass": pass_name, "note": f"Round 24 — {pass_name}"}, save_path)
...
# ═══════════════════════════════════════════════
TARGET      = args.target
floor       = 3.935445   # Round 23 pass3 best — this is what we beat
start_total = time.time()

print(f"Starting Round 24 — descent from {prev_loss:.6f}")
print("Farmer logic. Three passes. Two rewinds.")
print("Token trail active. training_top5_planes clean from epoch 1.")
print("Gap 2.33 — watch UNK slope close.")
print()
...
if loss1 < floor:
    ...
    torch.save({"model_state": state1, "best_loss": floor,
                "note": "Round 24 Pass 1 — FLOOR BROKEN"}, word_path)
...
if loss2 < floor:
    ...
    torch.save({"model_state": state2, "best_loss": floor,
                "note": "Round 24 Pass 2 — FLOOR BROKEN"}, word_path)
    print(f"  FLOOR BROKEN: {floor:.6f}")
...
if loss3 < floor:
    ...
    torch.save({"model_state": state3, "best_loss": floor,
                "note": "Round 24 Pass 3 — FLOOR BROKEN"}, word_path)
    print(f"  FLOOR BROKEN: {floor:.6f}")
...
broke_floor  = overall_best < 3.935445

print()
print("═" * 60)
print()
print(f"Round 24 complete.")
...
print(f"Previous floor: 3.935445")
if broke_floor:
    print(f"FLOOR BROKEN.   {overall_best:.6f} < 3.935445")
    print(f"Improvement:    {3.935445 - overall_best:.6f}")
else:
    print(f"Gap remaining:  {overall_best - 3.935445:.6f}")
    print(f"Round 25 loads: {pass3_path.name}  ({loss3:.6f})")
print()
print("Token trail sealed: /tmp/aria-token-trail.jsonl")
print("  python3 aria-core/diagnostics/token_trail.py --round 24 --all")
print()
print("Post-round baseline (run these three — same prompts as before-24):")
print("  python3 aria-core/inference_trace.py --prompt 'hello aria i am anthony i built you'")
print("  python3 aria-core/inference_trace.py --prompt 'aria what is the structure of a thought'")
print("  python3 aria-core/inference_trace.py --prompt 'aria what do you want to say'")
print()
