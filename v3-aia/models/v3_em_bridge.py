...
class V3EMBridge:
    ...
    def process(self, text: str) -> dict:
        ...
        # Clear stale values for domains with no current-cycle tokens —
        # a silent domain must not pollute the centroid with old am_center.
        for domain in self.field:
            ...
            if tokens:
                ...
                # No tokens this cycle — clear so centroid reflects live field only
                self.field[domain]["am_center"] = 0.0
                self.field[domain]["fm_spread"] = 0.0
